...
export FOX_USE_SAMSUNG_SPECIAL=0
...
function download_magisk(){
	...
	if [ -n "${DEST}" ]; then
		if [ ! -e ${DEST} ]; then
			...
			local LATEST_MAGISK_URL=$(curl -sL https://github.com/topjohnwu/Magisk/releases/latest | grep browser_download_url | grep Magisk- | cut -d : -f 2,3 | tr -d '"')
